...
router.get('/summary', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [lowStockCount, unpaidSalesCount, pendingPurchasesCount, pendingApprovalsCount, expiringProductsCount, customers] = await Promise.all([
    ...
    const total = lowStockCount + unpaidSalesCount + pendingPurchasesCount + pendingApprovalsCount + overLimitCustomersCount + expiringProductsCount;
